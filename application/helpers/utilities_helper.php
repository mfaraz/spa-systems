...
if (!function_exists('form_select')) {
	...
	function form_select($name, $items, $table, $selected = 0) {
		...
		foreach ($items as $item) {
            if($table == 'referrer' || $table == 'member'){
                $option[$item->$id] = ucfirst($item->firstname." ".$item->lastname);
            }else{
...
if (!function_exists('form_sex')) {
	...
	function form_sex($selected = '') {
		...
		return form_dropdown('sex', $option, $selected, 'class="form-control input-sm" id="gender"');
	}
...
if (!function_exists('javascripts')) {
	...
	function javascripts() {
		...
		//$scripts .= script_tag(JS_PATH . 'jquery.min.js') . "\r\n";
